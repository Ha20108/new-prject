...
def hotspot(request, pk):
                ...
                while i < len(images):
                    ...
                    for j in range(num_rows):
                        for k in range(num_columns):
                            ...
                            hotsadd.add(name=userm, profile=profile, comment=str(datetime.datetime.now()), limit_uptime=f'{uptime}h', limit_bytes_total= str(total_limit *1024*1024))


                            # إضافة الرقم إلى الصورة في ملف PDF
                            pdf_canvas.setFont('Helvetica', font_size)
                            pdf_canvas.setFillColorRGB(0, 0, 0)
                            pdf_canvas.drawString( x + text_x/2     , y + text_y/2   , userm , )
    ...
    return render(request, 'hotspot1.html', {'form': form ,'profile_choices':profile_choices })
...
def hotspot_create(request):
        if request.method == 'POST':
            ...
            while i < len(images):
                ...
                for j in range(num_rows):
                    for k in range(num_columns):
                        ...
                        pdf_canvas.drawImage(ImageReader(img_bytes), x, y, width=image_width, height=image_height)
                        userm = str(random.randint(10**(num_digits-1), 10**num_digits-1))
                        txt_file.write( f'/ip hotspot user add comment={comment} limit-bytes-total={limit} limit-uptime={uptime}h name={userm}\ profile={profile} \n')
            ...
            # إنشاء ملف مضغوط يحتوي على الملف النصي وملف الـPDF
            zip_filename = 'hotspot.zip'
            with zipfile.ZipFile(zip_filename, 'w') as zip_file:
                zip_file.writestr('hotspot.txt', txt_file.getvalue())
                zip_file.writestr('hotspot.pdf', pdf_bytes.getvalue())


            # تحميل الملف المضغوط كاستجابة من الخادم
            with open(zip_filename, 'rb') as zip_file:
                response = HttpResponse(zip_file, content_type='application/zip')
                response['Content-Disposition'] = 'attachment; filename="hotspot.zip"'
                return response
